...
resource "google_container_node_pool" "primary_nodes" {
  name       = google_container_cluster.sre_cluster.name
  location   = var.location
  cluster    = google_container_cluster.sre_cluster.name
  node_count = var.node_count
 # depends_on = [google_container_cluster.sre_cluster] #added newly
...
data "google_project" "project" {
  ...
  #service_account = "sre-terraform-tf@ringed-almanac-381213.iam.gserviceaccount.com"   #need to take a look at this 
}
